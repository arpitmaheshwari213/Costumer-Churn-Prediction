...
class PreprocessorState:
    ...
    def save(self, file_path: str):
        ...
        self.logger.info("Saving Preprocessor")
        if file_path:
            try:
                path = Path(file_path)
                ...
                self.logger.info(
                    f"[LOG] Preprocessor saved successfully at file_path: {file_path}"
                )
                return True
            except Exception as e:
                self.logger.exception(
                    f"[ERROR] Failed to save the preprocessor at file_path: {file_path} with Error: {e}"
                )
                raise
        else:
...
class FeatureExtraction:
    ...
    def fit_transform(self, dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info("Fit & Transform Started")
        ...
        try:
            ...
            X_processed = pd.DataFrame(
                data=X_transformed, columns=feature_names, index=X.index
            )
            self.logger.info(f"Shape of processed dataframe : {X_processed.shape}")

            preprocessor_state_obj = PreprocessorState(
                transformer=preprocessor, target_col=None, logger=self.logger
            )
            self.preprocessor_state_obj = preprocessor_state_obj
        except Exception as e:
            self.logger.exception(
                f"Feature Engineering Error during fit transform, {str(e)}"
            )
            raise
    ...
    def transform(self, dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            feature_names = self.sanitize_names(
                self.preprocessor_state_obj.transformer.get_feature_names_out()
            )
            self.logger.info(f"Feature Names {feature_names}")
            ...
            self.logger.exception(
                f"Feature Engineering Error during transform - {str(e)}"
            )
            raise
